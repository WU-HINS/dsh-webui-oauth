...
- **Unified "browser-influenced origin" switch (0.4.1)**: two redirect sites trust an origin **supplied by the requester (the browser)** — ① the OIDC callback base (the login page reports `location.origin`); ② the post-login core token redirect (authority comes from the request `Host`). Both **share one switch and one allowlist**, configured in the dsh settings `remote-web-ui` section (deployment level):
  ...
  remote-web-ui:
    ...
    trustedOrigins:                                 # allowlist honoured when on (or when the switch is omitted)
  ...
  - An empty `trustedOrigins` means *unconstrained* (legacy behavior); a non-empty list matches by **normalized origin** (scheme + host + port) with exact equality and **no wildcards** — a wildcard would reopen the loose-matching hole. A miss also falls back to `publicBaseUrl`.
  - The effective values are exposed on `/dsh-webui-oauth/status` as the `trustedOrigin` field (`trustBrowserOrigin`, `trustedOrigins`, `configuredOrigin`).
